 ...
 * own, so they are recognized without becoming bounds. The account's credit
 * balance can refuse paid requests first, but this endpoint does not report
 * it, so it is named as a caveat rather than modeled.
 */
function openrouterSemantics(
  ...
  return knownSemantics(
    ...
    "OpenRouter's key spend limit bounds every model served through the key, so effective remaining is the limit window's remaining share. The daily, weekly, and monthly usage windows are uncapped spend meters over the same credit spend and add no bound of their own. Paid requests are also refused once the account's credit balance runs out, which the key-status endpoint does not report.",
  );
}
