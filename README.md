...
OpenRouter's key spend `limit` window is the one enforced bound the key-status endpoint reports - a key whose remaining limit reaches zero is refused - so it is reported as one `all_models` scope. The `usage_daily`, `usage_weekly`, and `usage_monthly` windows are uncapped spend meters over the same credit spend, recognized without becoming bounds, so they never lower or mask the limit's headroom. The endpoint names the limit's reset cadence only as prose (`daily`, `weekly`, `monthly`) with no reset timestamp, so a recognized cadence contributes a trusted cycle duration where one exists (never for a calendar month) while the cycle phase, pace, runway, and selection stay unknown; an unrecognized cadence leaves the limit a plain credit cap. An unfamiliar OpenRouter window stays named in `unresolvedWindowIds` and turns the provider's semantics `partial`.
...
| OpenRouter             | Reads the `key` of Pi's `openrouter` `api_key` entry and reports the key's `limit` window (`limit_remaining` against `limit`, with `spentUsd`/`limitUsd`) plus `usage_daily`, `usage_weekly`, and `usage_monthly` spend meters from the first-party key-status endpoint. A `daily` or `weekly` `limit_reset` cadence contributes its trusted duration (86,400s or 604,800s); `monthly` and unrecognized cadences invent none, and the endpoint reports no reset timestamp, so pace stays `unknown`. A `null` limit is an unlimited key reported as usage meters only.                                                                                                                                                                                                                                                                                                                                                                                        |
...
- OpenRouter cache data is retired only on a definitive credential failure: an absent or invalid Pi entry, an unparseable auth file, or an HTTP 401/403 rejection. An auth file that exists but cannot be read is an indeterminate local failure rather than a sign-out, so it reports `state.status: error` and stays cache-eligible. Timeout, network, 408, 429, 5xx, and oversized-response failures may reuse a formerly fresh snapshot, with each window age-bounded by its own duration or period (a day for the daily meter, a week for the weekly, a month for the monthly and for a limit without a trusted duration).
