...
| `spend[]`      | **Emitted only when non-empty.** One row per fresh window that reports a dollar figure: `provider`, `window`, `spentUsd`, `limitUsd`. `limitUsd` is `none` for a meter with neither a limit nor a percentage and `unknown` for a capped window whose limit is unreported.                        |
...
| `unresolved_windows`                                    | `all`   | `quotaSemantics.unresolvedWindowIds`: unfamiliar, missing, or unreported vendor bounds not folded into any scope's effective percentage.                                                  |
...
OpenRouter's key spend `limit` window is the one bound the key-status endpoint reports - a key whose remaining limit reaches zero is refused - so it bounds the `all_models` scope. Paid requests are also bounded by the account's credit balance, which that endpoint does not report, so OpenRouter semantics are always `partial` with `account_credits` named in `unresolvedWindowIds`: the scope's effective percentage, runway, and selection stay unknown rather than presenting the limit's share as authoritative, while per-window and scope pace are still reported. The `usage_daily`, `usage_weekly`, and `usage_monthly` windows are uncapped spend meters over the same credit spend, recognized without becoming bounds. The endpoint names the limit's reset cadence only as prose (`daily`, `weekly`, `monthly`); OpenRouter documents resets at midnight UTC with Monday-to-Sunday weeks, so a recognized cadence and each usage meter carry the current UTC day, week, or calendar month as `startsAt`/`resetsAt`, and an unrecognized cadence leaves the limit a plain credit cap. An unfamiliar OpenRouter window is also named in `unresolvedWindowIds`.
