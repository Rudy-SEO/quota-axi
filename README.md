...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, OpenRouter, and Antigravity (`agy`) quota windows in one [AXI](https://axi.md)-shaped call.
...
| Flag                                                                                      | Description                                                               |
| ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,openrouter` | Scope providers                                                           |
...
- OpenRouter cards show dollars beside percentages: the key `limit` window keeps its bar and adds a note such as `day: $20.00 of $80.00 spent`, each uncapped usage meter renders under its full label (`day usage`, `week usage`, `month usage`) with `$X spent` in place of an empty bar, and the card notes that the account credit balance is not reported. No other provider's dollar windows change rendering.
...
| `spend[]`      | **OpenRouter only, emitted only when non-empty.** One row per fresh OpenRouter window: `provider`, `window`, `spentUsd`, `limitUsd`. `limitUsd` is `none` for an uncapped usage meter.                                                                                                           |
...
| `unreported_bound`                                      | `all`   | A fresh OpenRouter reading: `detail` `account_credits` names the account credit balance, which can refuse paid requests before the key limit does but is not reported by the key-status endpoint. |
...
OpenRouter's key spend `limit` window is the one bound the key-status endpoint reports - a key whose remaining limit reaches zero is refused - so it is reported as one known `all_models` scope with its effective percentage, runway, and selection. Paid requests are also refused once the account's credit balance runs out, which that endpoint does not report; that caveat is named in `quotaSemantics.description` and as an `unreported_bound` `attention[]` row rather than modeled. The `usage_daily`, `usage_weekly`, and `usage_monthly` windows are uncapped spend meters over the same credit spend, recognized without becoming bounds, so they never lower or mask the limit's headroom. The endpoint names the limit's reset cadence only as prose (`daily`, `weekly`, `monthly`); OpenRouter documents resets at midnight UTC with Monday-to-Sunday weeks, so a recognized cadence and each usage meter carry the current UTC day, week, or calendar month as `startsAt`/`resetsAt`, and an unrecognized cadence leaves the limit a plain credit cap. An unfamiliar OpenRouter window stays named in `unresolvedWindowIds` and turns the provider's semantics `partial`.
...
- Prefer trusted `startsAt` + `resetsAt` pairs (Grok's provider-reported current period; Cursor's monthly billing cycle, whose start comes from the payload's cycle start or the previous renewal date; OpenRouter's vendor-documented UTC day, Monday week, or calendar month).
...
| OpenRouter             | Reads the `key` of Pi's `openrouter` `api_key` entry and reports the key's `limit` window (`limit_remaining` against `limit`, with `spentUsd`/`limitUsd`) plus `usage_daily`, `usage_weekly`, and `usage_monthly` spend meters from the first-party key-status endpoint. A `daily`, `weekly`, or `monthly` `limit_reset` cadence, and each usage meter, resolves to the current UTC day, Monday-to-Sunday week, or calendar month as `startsAt`/`resetsAt`; an unrecognized cadence invents none. A `null` limit is an unlimited key reported as usage meters only.                                                                                                                                                                                                                                                                                                                                                                                          |

### Model catalog and `models`

`quota-axi models [--intelligence high|medium|low] [--sort runway] [--provider ...] [--json|--full]` joins a reviewed catalog of native Claude, Codex, Grok, and Kimi models to the provider's effective quota evidence. It queries those four catalog-backed providers by default and accepts only those providers in an explicit models scope. Cursor and Copilot are excluded from this first catalog because their hosted model availability is plan-dependent; Copilot's quota relationships are also currently unknown. Z.AI, Alibaba, OpenCode Go, OpenRouter, and Antigravity report quota but have no reviewed catalog entries yet, so they are not `models` providers either.
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `pi:openrouter`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
| OpenRouter     | Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) for the literal `key` of the `openrouter` `api_key` entry                                                                                                                                                                                                                                                                                                                                            |
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, OpenRouter, Antigravity | none                              | read-only; see the per-provider notes below           |
